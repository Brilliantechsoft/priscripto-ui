...
export const fetchDoctors = createAsyncThunk<Doctor[], string | undefined>('doctors/fetchDoctors', async (speciality,{ dispatch }) => {
...
const doctorSlice = createSlice({
  name: 'doctors',
  initialState,
  reducers: {   filterDoctors: (state, action: PayloadAction<string | undefined>) => {
    if (action.payload) {
      const specialization = action.payload.toLowerCase();
      state.filteredDoctors = state.allDoctors.filter(doc => doc.specialization.toLowerCase() === specialization);
    } else {
